...
// Define owner/admin-only AWS onboarding material.
export interface CloudAccountOnboardingApiResponse
  extends CloudAccountApiResponse {
  // Store CloudOps-generated confused-deputy protection value.
  external_id: string;

  // Store the CloudOps AWS identity the customer must trust.
  platform_principal_arn:
    | string
    | null;

  // Provide a predictable recommended IAM role name.
  suggested_role_name: string;

  // Return the exact IAM trust policy generated by the backend.
  trust_policy:
    | Record<string, unknown>
    | null;

  // Tell the UI whether production platform identity is configured.
  onboarding_ready: boolean;
}


// Define successful AWS STS validation result.
export interface CloudAccountValidationApiResponse {
  connected: boolean;
  account_id: string;
  caller_arn: string;
  message: string;
